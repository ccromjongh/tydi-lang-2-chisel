...
class {{ name | capitalize }}(r: Boolean = false) extends PhysicalStreamDetailed(e=new {{type.value.stream_type.name | capitalize}}, n={{type.value.throughput | int}}, d={{type.value.dimension}}, c={{type.value.complexity}}, r=r^{{'true' if type.value.direction=='Reverse' else 'false'}}, u={% if type.value.user_type.type == LogicType.null %}Null(){% else %}new {{type.value.user_type.name | capitalize}}{% endif %})
{%- endmacro %}
...
object {{ type.name | capitalize }} {
    def apply(r: Boolean = false): {{ type.name | capitalize }} = Wire(new {{ type.name | capitalize }}(r=r))
}
{%- endmacro %}
